...
class Model(nn.Module):
    def __init__(self, entrop, var, emb_len, input_shape, hidden1, hidden2, params):
    ...
    def evaluate(self, X_train_not_emb, X_train_emb, labels_train, X_val_not_emb, X_val_emb, labels_val, label_name):
        y_train_predict = self.fit_transform(X_train_not_emb, X_train_emb)
        y_val_predict = self.fit_transform(X_val_not_emb, X_val_emb)
        ...
        return auc_score_val, auc_score_train
...
def train_model(X_train, y_train, X_val, y_val, model_params):
    ...
    for i, label in enumerate(labels_options):
        ...
        # build datasets
        X_train_dataset = Dataset(X_train_not_emb, X_train_emb, y_train[i])
        X_val_dataset = Dataset(X_val_not_emb, X_val_emb, y_val[i])
        ...
        model = Model(entrop=entrop_vec[i], var=variance_vec[i], emb_len=ntypes_embedding,
        ...
        print("Evaluation")
        auc_val, auc_train = model.evaluate(X_train_not_emb, X_train_emb, y_train[i], X_val_not_emb, X_val_emb, y_val[i], label)
        auc_val_vec[label] = auc_val
